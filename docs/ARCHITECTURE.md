...
`port_raise_priv()` / `port_drop_priv()` are the arch-level implementations these wrap; user code never calls the `port_*` versions directly, same opaque-boundary rule as everywhere else in this doc.
